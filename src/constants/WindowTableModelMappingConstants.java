package constants; 

 public class WindowTableModelMappingConstants { 

 final public static String WINDOW_NAME  = "WindowTableModelMapping";
 final public static String WINDOWNAME   = "WindowName";
 final public static String BEANNAME   = "BeanName";

 final public static String METHODNAME   = "MethodName";
 final public static String COLUMNNAME   = "ColumnName";
 final public static String COLUMNDATATYPE   = "ColumnDataType";
 final public static String COLUMNDISPLAYNAME   = "DisplayColumnName";
 
 final public static int SPLITWINDOWLOCATION =100;
 final public static int WINDOWWIDTH = 200;
 final public static int WINDOWHIGHT = 120;
 final public static String CUSTOMCOLUMNNAME ="CustomColumnName";

 final public static String CUSTOMMETHODNAME ="CustomMethodName";
public static final String SEARCHTEXTBOX = "WindowTableModelMappingSearchTextBox";
public static final String CUSTOMDATATYPE = "CustomColumnDataType";
public static final String ISCOMBOX = "IsCombobox";

}